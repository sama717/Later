...
function Login() {
  ...
  async function onSubmit(values: LoginFormValues) {
    const success = await signInWithPassword(values.email, values.password);
    if (success) navigate("/");
  }

  return (
    <AuthLayout backgroundImage={loginBg}>
      <h1 className="text-3xl font-medium text-foreground text-center">Welcome back</h1>
      <p className="text-muted-foreground mt-1 text-center">Log in to continue</p>
